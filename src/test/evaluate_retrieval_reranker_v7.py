...
# [NEW] HyDE 생성 온도 - 기본값을 0(결정적)으로 변경했습니다.
# 기존 0.3은 같은 질의라도 실행마다 다른 가상 답변을 생성해, 코드 변경이 전혀 없어도
# eval 결과(특히 law hard case들)가 실행마다 들쭉날쭉했던 주요 원인으로 추정됩니다.
# 회귀 비교(A/B) 목적의 eval에는 재현성이 중요하므로 0으로 고정합니다.
...
# 잡히는 경우가 있어(HARD_PREC_04 등) 이를 완화하기 위한 소폭 가중치입니다.
# chunk_id에 "_head" 패턴은 prec에만 존재하므로 다른 doc_type에는 영향 없습니다.
PREC_HEAD_BOOST: float = float(os.getenv("PREC_HEAD_BOOST", "1.15"))
...
# [NEW] lstrm 보일러플레이트 위임규정 필터 활성화 여부
# title이 "법령용어: 대통령령/총리령/부령/OO령으로 정하는 ~" 패턴이면 실제 법령용어가
# 아니라 입법 관용구(위임 규정 문구)가 잘못 추출된 것으로 간주해 검색 후보에서 제외합니다.
# (조사 근거: 전체 lstrm 110,573건 중 13,473건, 6,079개 title이 이 패턴)
FILTER_LSTRM_BOILERPLATE: bool = os.getenv("FILTER_LSTRM_BOILERPLATE", "true").lower() == "true"

# [NEW] lstrm 출처 없는 영단어 gloss 필터 활성화 여부
# content가 "출처: "로 끝나고 그 뒤에 아무 내용이 없으면(=출처 공란), 실제 법적
# 정의가 아니라 한영 법률용어 대조사전류 gloss 항목으로 간주해 검색 후보에서 제외합니다.
# (조사 근거: 전체 lstrm 110,573건 중 25,781건(약 23%)이 이 패턴 - boilerplate 필터보다 규모가 큼)
...
def execute_advanced_search(
    ...
    # [NEW] lstrm 보일러플레이트 위임규정("대통령령으로 정하는 ~" 등) 제외 필터
    # 법령용어가 아니라 입법 관용구가 잘못 추출된 것이므로 검색 후보에서 제외합니다.
    ...
    # [NEW] lstrm 출처 없는 영단어 gloss 제외 필터
    # content가 "출처: "로 끝나고 뒤에 아무것도 없으면 한영 대조사전류 gloss로 간주해 제외합니다.
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            # [NEW] 후보군이 0건이면 원인 진단을 위해 vector_search/text_search 각각의
            # 결과 건수를 별도로 재조회해서 로깅합니다. (HARD_LAW_07 등에서 재현된
            # "검색된 Top-5 IDs : []" 현상 원인 추적용)
            if not candidates:
                cur.execute(f"""
                ...
                logger.warning(
                    ...
                    f"embedding_input='{embedding_input[:80]}...' "
                    f"doc_type_filter='{doc_type_filter}' "
                    f"vector_dims={len(query_vector)} "
